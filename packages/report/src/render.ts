...
const MSG = {
  en: {
    ...
    reproTitle: 'How to reproduce this',
    reproBody: 'This report was generated by FasterGEO, which is open source. Every formula behind these numbers is published in METHODOLOGY.md.',
    reproSamples: 'Every rate above is recomputable from the verbatim answers in Answer Replay below.',
    reproSelf: 'Run it yourself',
    reproNoTerminal: 'Would rather not use a terminal',
    reproSite: 'fastergeo.co — paste a URL for a free crawler\'s-eye audit, no sign-up',
  ...
  zh: {
    ...
    reproTitle: '这份报告怎么复现',
    reproBody: '本报告由开源的 FasterGEO 生成。上面这些数字背后的每条公式都公开在 METHODOLOGY.md。',
    reproSamples: '上面每个比率都能用下面「答案回放」里的原文重新算一遍。',
    reproSelf: '自己跑一遍',
    reproNoTerminal: '不想碰终端',
    reproSite: 'fastergeo.co —— 粘贴网址，免费看一次 AI 爬虫眼中的你，不用注册',
...
/* Reproduction block — the last thing a forwarded report says. */
.repro{background:var(--well)}
.repro p{margin:0 0 14px;color:var(--dim);font-size:13px;max-width:70ch}
.repro-ways{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px}
.repro-ways>div{background:var(--card);border:1px solid var(--line);border-radius:8px;padding:13px 15px}
.repro-k{display:block;font:500 10px ui-monospace,"IBM Plex Mono",Menlo,monospace;letter-spacing:.12em;
text-transform:uppercase;color:var(--faint);margin-bottom:6px}
.repro code{font:12.5px ui-monospace,"IBM Plex Mono",Menlo,monospace;word-break:break-all}
...
/**
 * Written for the person who received this report rather than the one who ran
 * it — a developer forwards the file to whoever owns the brand, and until now
 * that reader hit the end with nowhere to go.
 *
 * Framed as reproduction instructions, not a pitch, because reproducibility is
 * the actual claim: anyone can re-derive these numbers, which is the whole
 * argument for trusting them. The hosted line promises only the free audit that
 * exists today; nothing here may describe a capability that has not shipped.
 */
function reproSection(input: ReportInput, m: M): string {
  const root = input.audit?.root ?? 'https://yoursite.com';
  // Only point at Answer Replay when it is actually in the document — a
  // reference to a section that did not render is a small lie in a report whose
  // entire argument is that you can check it.
  const hasReplay = (input.samples?.length ?? 0) > 0;
  return `<section class="repro">
    <h2>${m.reproTitle}</h2>
    <p>${m.reproBody}${hasReplay ? ' ' + m.reproSamples : ''}</p>
    <div class="repro-ways">
      <div><span class="repro-k">${m.reproSelf}</span>
        <code>npx fastergeo report --root ${esc(root)}</code></div>
      <div><span class="repro-k">${m.reproNoTerminal}</span>
        <span>${esc(m.reproSite)}</span></div>
    </div>
  </section>`;
}
